...
> [`../template/docs/`](../template/docs/), the existing reference material in
> [`../spec/`](../spec/), [`../file-structure.md`](../file-structure.md), and the plan files in
> [`../plans/`](../plans/).
>
...
repo root**, scaffolded from [`../template/docs/`](../template/docs/) and initialized with the full
...
- Read the template thoroughly first: [`../template/docs/README.md`](../template/docs/README.md) and
...
- **Traceability is the point.** Every spec cites the ADR(s) and research that informed it; every
  ...
  [`../template/docs/procedures/verify-traceability.md`](../template/docs/procedures/verify-traceability.md).
...
- **Files:** copy [`../template/docs/`](../template/docs/) → repo-root `docs/`. Keep verbatim:
...
- **Context:** Source the content from [`../spec/00-overview.md`](../spec/00-overview.md) and
  [`../README.md`](../README.md): *What we're building* (the event-driven local-first trading
...
- **Context:** From [`../spec/10-open-questions.md`](../spec/10-open-questions.md): Q1 (real-vs-paper),
...
- **Context:** Mine the specs for the decisions and write one ADR each (use the ADR template in
  [`../template/docs/adr/README.md`](../template/docs/adr/README.md): Context/Decision/Rationale/
  Consequences/Alternatives). Suggested set (number sequentially, adjust as needed):
  - `0001-rust-modular-monolith-with-satellite-collectors` (from [`01-architecture`](../spec/01-architecture.md))
  - `0002-decimal-money-newtypes-no-f64` (from [`02`](../spec/02-data-model.md)/[`03`](../spec/03-data-engineering.md))
  - `0003-nats-jetstream-event-fabric` (from [`09`](../spec/09-tech-stack.md))
  - `0004-storage-split-postgres-clickhouse-parquet-redis` (from [`07`](../spec/07-storage-and-replay.md))
  - `0005-single-risk-gate-chokepoint-and-kill-switch` (from [`05`](../spec/05-execution-and-risk.md))
  - `0006-three-system-broker-architecture-coinbase-alpaca-market-simulator` (resolves Q-1 AND Q-2:
    ...
    must exist for all assets/domains; from [`05`](../spec/05-execution-and-risk.md)/[`10`](../spec/10-open-questions.md))
  - `0011-demand-driven-data-engines-no-auto-start` (data pipelines start ONLY when a strategy or UI
    ...
    [`03`](../spec/03-data-engineering.md)/[`04`](../spec/04-strategy-system.md))
  - `0007-freeze-strategy-definition-format-v1` (resolves Q-3, from [`04`](../spec/04-strategy-system.md))
  - `0008-available-time-ordering-and-same-builders-live-and-replay` (from [`03`](../spec/03-data-engineering.md)/[`07`](../spec/07-storage-and-replay.md))
  - `0009-append-only-raw-event-archive-as-ground-truth` (from [`07`](../spec/07-storage-and-replay.md))
  - `0010-three-front-doors-one-canonical-strategy-json` (from [`04`](../spec/04-strategy-system.md)/[`08`](../spec/08-mcp-server.md))
...
- **Files:** `docs/specs/<TYPE>-<NNN>-*.md` + `docs/specs/README.md` index. Use the spec template and
  procedure in [`../template/docs/procedures/add-spec.md`](../template/docs/procedures/add-spec.md).
- **Context:** Recommended mapping (split further if a file would exceed ~7 cohesive sections):
  - [`02-data-model`](../spec/02-data-model.md) → `DATA-001` event-envelope-and-payloads,
    `DATA-002` instrument-metadata, `DATA-003` timestamps-and-identity.
  - [`03-data-engineering`](../spec/03-data-engineering.md) → `COMP-001` data-quality-and-ingestion
    (quarantine, dedup, watermarks/revisions, reconciliation policy).
  - [`04-strategy-system`](../spec/04-strategy-system.md) → `FEAT-001` strategy-system +
    `DATA-004` strategy-definition-format (the frozen 1.0 contract).
  - [`05-execution-and-risk`](../spec/05-execution-and-risk.md) → `COMP-002` execution-and-risk-gate.
  - [`06-ui-and-streaming`](../spec/06-ui-and-streaming.md) → `COMP-003` ui-streaming-gateway.
  - [`07-storage-and-replay`](../spec/07-storage-and-replay.md) → `COMP-004` storage-and-replay.
  - [`08-mcp-server`](../spec/08-mcp-server.md) → `INTG-001` mcp-server.
  - A `SYS-001` system-overview spec linking all of the above (diagram + cross-reference index),
    sourced from [`01-architecture`](../spec/01-architecture.md).
  - [`09-tech-stack`](../spec/09-tech-stack.md) becomes a **research brief** (P A-T06) + the ADRs,
    not a spec. [`12-glossary`](../spec/12-glossary.md) becomes `docs/glossary.md` (P A-T09).
...
- **Context:** The first brief sources [`../spec/09-tech-stack.md`](../spec/09-tech-stack.md)
...
- **Context:** Fill the template's sections (Overview, Components, Data Flow, External Dependencies,
  Key Decisions, Constraints) from [`../spec/01-architecture.md`](../spec/01-architecture.md). **Fold
  the entire enumerated structure from [`../file-structure.md`](../file-structure.md) into the
...
- **Files:** `docs/plans/rust-rewrite-master-plan.md` (from
  [`./00-master-plan.md`](./00-master-plan.md)); `docs/plans/phase-A-documentation.md` …
  `phase-7-cutover.md` (from this directory); `docs/plans/README.md` index rows. Follow
  [`../template/docs/procedures/add-plan.md`](../template/docs/procedures/add-plan.md).
...
- **Context:** `README.md` = project overview pointing into `docs/`. `AGENT.md` = agent operating
  ...
  [`../spec/12-glossary.md`](../spec/12-glossary.md). Note: the legacy root `AGENTS.md` from the
...
- **Files:** run [`../template/docs/procedures/verify-traceability.md`](../template/docs/procedures/verify-traceability.md)
