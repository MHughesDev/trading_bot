...
> **Self-contained execution doc.** You need only: this file, `docs/architecture.md`,
> `docs/plans/rust-rewrite-master-plan.md` §6 (definition of done), the live `docs/` workspace
> (stood up in Phase A), and `refactor_reference_docs/` — which is still present at root as the
> permanent reference anchor and is the **last thing deleted**, as the final act of this phase.
>
...
- **Goal:** Confirm every "decided mechanism" test from the spec is green (per
  [`../spec/10-open-questions.md`](../spec/10-open-questions.md) standing reminder).
...
- **Acceptance:** `legacy_python/` is gone; no file references it; `just test` and CI remain green;
  the repo tree matches [`../file-structure.md`](../file-structure.md) (no orphans, nothing missing).
- **Depends on:** P7-T02.
...
- **Context:** Walk [`../file-structure.md`](../file-structure.md): every enumerated crate/app/file
...
- **Acceptance:** zero orphans, zero missing enumerated files (or documented, justified deviations
  with `file-structure.md` updated to match reality).
- **Depends on:** P7-T03.
...
- **Files touched in `docs/`:**
  ...
  - **`docs/open-questions.md`** — close any remaining `Open` questions resolved during the build
    (broker/venue Q-2, watermark defaults Q-7, retention Q-8); add the resolving ADR or research link.
...
- **Context:** Phase A scaffolded the workspace with design intent; this task makes it reflect
  actuality. Follow `docs/procedures/add-spec.md` (the spec lifecycle steps) and
  `docs/procedures/verify-traceability.md` (the closing check). Operational procedures follow the
  authoring template in `docs/procedures/README.md`.
...
- [ ] `docs/plans/rust-rewrite-master-plan.md` §6 definition of done is satisfied.
