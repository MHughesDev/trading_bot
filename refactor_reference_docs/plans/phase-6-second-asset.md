...
> and the specs — especially [`02-data-model.md`](../spec/02-data-model.md) (instrument metadata),
> [`05-execution-and-risk.md`](../spec/05-execution-and-risk.md) (stocks vs crypto), and
> [`03-data-engineering.md`](../spec/03-data-engineering.md) §8 (trust tiers).
...
- Phases 1–4 complete (spine, money safety, UI optional, strategies/backtest). The crypto collector
  ...
  [`../spec/10-open-questions.md`](../spec/10-open-questions.md) Q2).
...
- **Context:** Per [`../spec/02-data-model.md`](../spec/02-data-model.md) §instrument metadata and
  [`../spec/05-execution-and-risk.md`](../spec/05-execution-and-risk.md) §stocks vs crypto: equities
  have session+auction `trading_hours`, can halt (`halt_behavior`), quote precision 2dp, lot size
  rules, `trust_tier = Regulated`. Crypto stays 24/7. Any `TradingSchedule` extension must remain
  expressive enough for both (24/7 and session) without a core code branch.
...
- **Context:** Per [`../spec/10-open-questions.md`](../spec/10-open-questions.md) Q2: build the
  second collector deliberately differently (Alpaca uses REST+WS with different auth/snapshot model
  vs Kraken's WS v2) so the lane/metadata abstraction is *proven*. It normalizes Alpaca's equity
  feed shapes into the same `TradePayload`/`QuotePayload`/`OrderBookPayload`/`BarPayload` on the
  same `market.*` lanes, with the same quarantine + gap-detection machinery from `crates/collectors`.
  It must be hours/halt aware (publishing halt/session state into metadata-relevant signals). Add the
  Alpaca equity routing rule to `crates/venue-router` so this collector also starts **only on demand**.
...
- **Context:** Per [`../spec/05-execution-and-risk.md`](../spec/05-execution-and-risk.md) §stocks vs
  crypto: session/auction order acceptance, halt rejection, and settlement/shorting constraints live
  in the **adapter and metadata** (not core). The Alpaca paper adapter handles both crypto and equity
  orders — this task proves it for equity. Coinbase live equity trading is **post-Phase-6 scope**;
  the `coinbase.rs` stub is ready for it but not implemented here. The risk gate already reads
  `halt_behavior`/`trading_hours` from Phase 2 — confirm those checks fire for equity instruments.
