...
> and the specs in [`../spec/`](../spec/). This phase produces no business logic — it produces the
...
layout in [`../file-structure.md`](../file-structure.md): every crate and app exists as an empty-but-
...
- Pin external dependency versions **once** in `[workspace.dependencies]` (see
  [`../spec/09-tech-stack.md`](../spec/09-tech-stack.md)); member crates use `.workspace = true`.
...
- **Context:** `Cargo.toml` declares `members = ["crates/*", "apps/*", "xtask"]`, `resolver = "2"`,
  ...
  [`../spec/09-tech-stack.md`](../spec/09-tech-stack.md) pinned, `[workspace.lints]`, and tuned
...
- **Goal:** Create each crate under `crates/` as an empty compiling library with the module files
  enumerated in [`../file-structure.md`](../file-structure.md) present as empty stubs
...
- **Context:** Submodule files can be empty but must be declared so the path is reserved and the
  ...
  graph in `file-structure.md` allows — **except `builders`/`features`, which must NOT depend on
...
- **Context:** Each `main.rs` is wiring-only per `file-structure.md`; for now just
...
- **Context:** Model service choices on [`../spec/07-storage-and-replay.md`](../spec/07-storage-and-replay.md)
  and [`../spec/09-tech-stack.md`](../spec/09-tech-stack.md). `legacy_python/infra/docker-compose*.yml`
...
- [ ] Every crate in `crates/` and app in `apps/` from `file-structure.md` exists and compiles (including `venue-router`).
