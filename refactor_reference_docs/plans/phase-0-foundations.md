...
> and the specs in [`../spec/`](../spec/) — especially [`02-data-model.md`](../spec/02-data-model.md),
> [`03-data-engineering.md`](../spec/03-data-engineering.md), and [`04-strategy-system.md`](../spec/04-strategy-system.md).
>
...
- **Decision gate Q3** (strategy-format 1.0 freeze) is *produced* in this phase (P0-T08). The
  ...
  [`../spec/04-strategy-system.md`](../spec/04-strategy-system.md) and document them.
...
- **Context:** `pub struct Price(pub Decimal); pub struct Size(pub Decimal);` over `rust_decimal`.
  ...
  [`../spec/02-data-model.md`](../spec/02-data-model.md) §money types).
...
- **Context:** Variants `Regulated`, `CentralizedExchange`, `OnchainConfirmed`, `OnchainTentative`,
  `SocialDerived` (per [`../spec/03-data-engineering.md`](../spec/03-data-engineering.md) §8). Provide
  ordering so a strategy's `min_trust_tier` can be compared against an event's tier. serde.
...
- **Context:** `event_time` (Option), `observed_time`, `ingested_time`, `available_time` — all
  `DateTime<Utc>` (see [`../spec/02-data-model.md`](../spec/02-data-model.md) §timestamp model).
  Provide a helper to compute `available_time = max(window_close, observed) + watermark + processing_delay`.
...
- **Context:** Exactly the structs in [`../spec/02-data-model.md`](../spec/02-data-model.md) §v1
  payloads: `TradePayload`, `QuotePayload`, `OrderBookPayload` (`BookUpdateKind`, levels,
...
- **Context:** Exactly the struct in [`../spec/02-data-model.md`](../spec/02-data-model.md) §envelope:
  `event_id`, `event_type`, `schema_version`, `lane`, `instrument_id`, `venue_id`, `source`,
  `trust_tier`, the four timestamps, `sequence`, `correlation_id`, `causation_id`, `payload: T`.
  Generic over payload `T: Payload`.
...
- **Context:** Dedup-key forms from [`../spec/02-data-model.md`](../spec/02-data-model.md) §identity:
  `lane+instrument+venue+sequence+source`, `venue+exchange_trade_id`, `chain+tx_hash+log_index`
  (future). `lanes.rs` enumerates lane name constants + a typed lane enum from the lane list in
  [`../spec/01-architecture.md`](../spec/01-architecture.md) (`market.trades`, `market.quotes`,
  `market.orderbook.l2`, `market.bars.1s`, `market.bars.1m`, `features.technical`,
  `strategy.signals`, `orders.commands`, `orders.events`, `positions.events`, `quarantine`).
...
- **Context:** Exactly the struct in [`../spec/02-data-model.md`](../spec/02-data-model.md) §instrument
  metadata: `instrument_id`, `asset_class` (`Crypto`/`Equity`, extensible), `venue_id`,
...
- **Context:** Build out the sketch in [`../spec/04-strategy-system.md`](../spec/04-strategy-system.md)
...
- **Acceptance:** `strategy_def_schema.rs` deserializes the example JSON from
  [`../spec/04-strategy-system.md`](../spec/04-strategy-system.md) and re-serializes it stably;
  `definition_version` is `"1.0"`; module docs state the frozen grammar and the tighten-only rule.
...
- **Context:** `OrderRequest`/`OrderIntent` (with an **idempotency key** field — see
  [`../spec/05-execution-and-risk.md`](../spec/05-execution-and-risk.md) §idempotency),
  `OrderState` machine states (`accepted/submitted/partially_filled/filled/cancelled/rejected`),
  `Side`, `OrderType`. `Position`, `Balance`. `error.rs` seeds `NormalizeError`, `ValidationError`,
  `RiskRejection`. `lib.rs` re-exports the full public API.
...
- **Context:** Money columns are Postgres `NUMERIC` / ClickHouse `Decimal128(scale)` — **never
  float** (see [`../spec/02-data-model.md`](../spec/02-data-model.md) §money, §storage). ClickHouse
  tables use `ReplacingMergeTree` ordered on the dedup key, `ORDER BY (instrument, available_time)`,
  partitioned by month (see [`../spec/03-data-engineering.md`](../spec/03-data-engineering.md) §9,
  [`../spec/07-storage-and-replay.md`](../spec/07-storage-and-replay.md)). Instruments table mirrors
  P0-T07. `0005` includes the global `trading_enabled` flag + per-user risk limits.
...
- **Context:** Per [`../spec/07-storage-and-replay.md`](../spec/07-storage-and-replay.md): raw
  normalized events written **before any derivation**, append-only, immutable, partitioned
