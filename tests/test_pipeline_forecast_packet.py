...
def test_forecast_packet_disabled_in_legacy_without_flag() -> None:
    """With `legacy` mode and no packet flag, no ForecastPacket is attached."""
    pipe = DecisionPipeline(
        settings=AppSettings(decision_pipeline_mode="legacy", decision_forecast_packet_enabled=False)
    )
...
def test_routing_uses_ridge_by_default() -> None:
    ...
    _, fc_with_packet, _, _ = pipe2.step(
        "BTC-USD",
        feats,
        spread_bps=5.0,
        risk=risk,
...
def test_routing_packet_derives_forecast_output() -> None:
    s = AppSettings(
        decision_pipeline_mode="legacy",
        decision_forecast_routing_source="packet",
    )
    pipe = DecisionPipeline(settings=s)
    feats = {f"f{i}": float(i) * 0.01 for i in range(32)}
    feats["close"] = 50_000.0
    feats["volume"] = 1e6
    risk = RiskState()
    regime_out, fc, route, action = pipe.step(
        "BTC-USD",
        feats,
        spread_bps=5.0,
        risk=risk,
