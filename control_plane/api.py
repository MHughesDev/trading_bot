...
# --- Asset init pipeline (FB-AP-006 / FB-AP-007) ---
...
def post_asset_init(
    ...
    """
    Start a background per-asset init job (one global runner).

    Poll ``GET /assets/init/jobs/{job_id}`` for progress. First step pulls Kraken REST history
    (see ``orchestration.init_kraken_historical`` / ``NM_ASSET_INIT_BOOTSTRAP_*``).
    """
    sym = symbol.strip()
