...
ROWS: list[dict[str, str]] = [
    {
        ...
        "agent_task": (
            ...
            "Design narrative: The friendly replacement for the current red "
            "'Chart data failed: 500 Internal Server Error' box is a quiet, "
            "centered empty-state inside the chart area — a small outlined "
            "Material icon, one line in text-secondary reading 'No price "
            "history for <SYMBOL> yet', and the Initialize primary button "
            "directly underneath. No red, no alarm. When the backend DOES "
            "return a typed 503 envelope (storage_unavailable), surface it "
            "as a compact inline banner with the loss-red CSS var as the "
            "left border only, text in text-primary, and a small 'Retry' "
            "link. The 500-with-envelope case gets the same treatment but "
            "labeled 'Unexpected error'. No raw HTML error pages ever reach "
            "the operator."
        ),
        "affected_files": (
    ...
    },
    {
        ...
        "agent_task": (
            ...
            "Design narrative: A fixed 240px rail on the left of the canvas "
            "(#0B0F17 background), separated from the main area by a single "
            "hairline border in #1F2937 — never a drop-shadow. At the very "
            "top, a compact brand lockup in JetBrains Mono 13px reading "
            "'trading_bot' in text-primary with a small outlined mark to "
            "its left. A 1px divider in border color. Then five stacked "
            "primary links — Dashboard, Asset page, Account, Sign in, Sign "
            "up — each rendered as a single row: 20px Material Symbols "
            "Outlined icon on the left, Inter 14px label next to it, 12px "
            "vertical padding, 16px horizontal padding. Hover fills the "
            "row with the surface color (#111827) on a 150ms ease. The "
            "currently active page gets two treatments: a 2px accent bar "
            "(#6366F1) flush-left inside the row, and its label brightens "
            "to full primary text color. Inactive rows use text-secondary. "
            "Divider again. Then a collapsible 'Watching' header: "
            "uppercase 11px label in text-tertiary, letter-spacing 0.08em, "
            "with a right-aligned caret that rotates 90deg when expanded. "
            "Expanded rows each show the symbol in JetBrains Mono 13px on "
            "the left and the last price + delta as a right-aligned pill "
            "(gain-green or loss-red 8% opacity fill, full-color text). "
            "Clicking a row deep-links to /Asset?symbol=X. Divider. Then a "
            "'Holdings' section identical in structure — symbol left, then "
            "qty in mono, then unrealized P&L colored. Empty state is a "
            "single line of text-tertiary: 'None'. Absolutely nothing else "
            "in this sidebar — no Grafana URL, no QuestDB URL, no config "
            "symbol list, no duplicated default multipage nav."
        ),
        "affected_files": (
    ...
    },
    {
        ...
        "agent_task": (
            ...
            "toggle. "
            "Design narrative — THE VIBE: Quiet, confident trading "
            "infrastructure. Think Linear's clarity + Coinbase Advanced's "
            "restraint + a Bloomberg terminal that doesn't hate your eyes. "
            "No gradients, no drop-shadows, no emoji, no cartoon "
            "illustrations, no 'engagement' patterns. One accent color. "
            "Gain-green means up, loss-red means down, everything else is "
            "grayscale. "
            "COLOR HIERARCHY: Canvas #0B0F17 (almost-black, faint cool-blue "
            "undertone — deep enough to disappear without straining). "
            "Surface #111827 sits one step lighter as the card floor, "
            "separated by a 1px border in #1F2937 — never shadow. Text is "
            "a real three-step hierarchy: primary #F3F4F6 (soft white, not "
            "pure), secondary #9CA3AF (labels, captions), tertiary #6B7280 "
            "(hints, dividers, meta). The only saturated colors appear "
            "sparingly: accent #6366F1 (indigo, reserved for the one "
            "primary CTA per view and active-nav indicators), gain #22D3A0 "
            "(mint-green), loss #F87171 (coral-red). Amber #F59E0B is "
            "reserved strictly for risk warnings and should appear at most "
            "once per session. No color is decorative. "
            "TYPOGRAPHY: Inter for all UI (400 / 500 / 600). JetBrains "
            "Mono for every number, price, P&L, qty, timestamp, and "
            "ticker. Tabular numerals are non-negotiable wherever numbers "
            "change so columns don't jitter. Dashboard hero P&L is ~56px "
            "weight 600. Section headings are small (11px) weight 600 "
            "UPPERCASE letter-spacing 0.08em in text-tertiary — they "
            "label, they don't shout. Body is 14px weight 400. Captions "
            "12px weight 400 text-secondary. No italics. Underlines only "
            "appear on hover over actionable text links. "
            "RHYTHM: Everything sits on an 8px grid. Page outer padding "
            "32px. Card inner padding 24px, corner-radius 12px, border "
            "1px solid #1F2937, no box-shadow. Gap between cards 16px. "
            "MOTION: Barely there. 150ms ease-out hover fades only. No "
            "bouncing, no progress spinners where skeleton shimmers will "
            "do. Numbers fade in place on update, never flash. "
            "ICONS: Material Symbols Outlined only, 20px default, "
            "inherit current text color, one consistent stroke weight. "
            "Used only where they disambiguate — never decoratively. "
            "EMPTY STATES: No illustrations. One line of text-tertiary, "
            "optionally one primary CTA underneath. Never a red banner "
            "for a non-broken situation. "
            "CHROME TO HIDE: #MainMenu (three-dot), the Streamlit footer, "
            "the header bar including the Deploy button and hamburger, "
            "the 'Made with Streamlit' watermark. All removed via CSS in "
            "inject_global_css."
        ),
        "affected_files": (
    ...
    },
    {
        ...
        "agent_task": (
            ...
            "Design narrative: The dashboard is the operator's calm "
            "portfolio view — no noise, no controls competing with the "
            "numbers. Top-left: a small uppercase 11px text-tertiary "
            "eyebrow label reading 'PORTFOLIO'. Immediately under it, the "
            "cumulative P&L as the hero — JetBrains Mono ~56px weight 600 "
            "in text-primary, leading-tight. Under the hero number: the "
            "period delta and percent in the gain/loss color, 14px mono, "
            "e.g. '+$1,247.82  +2.14%'. Top-right of the same row: a "
            "compact segmented control 'Paper | Live'. The inactive side "
            "is just text-secondary on the canvas; the active side gets a "
            "surface fill (#111827), text-primary, and a subtle 1px "
            "border. Tap either side to switch which ledger feeds the "
            "chart. This toggle is VIEW-ONLY — it changes the chart, it "
            "does not change order routing. No global live/paper kill "
            "switch anywhere on this page. "
            "HERO CHART: Full-width line chart below the hero numbers, "
            "roughly 320-380px tall. Background is the canvas color so "
            "the chart feels part of the page, not a card. Line color is "
            "gain-green if the visible range is net positive over its "
            "start, loss-red if net negative — judged per re-render. "
            "Under the line, a soft area fill at 8% opacity of the same "
            "color. Gridlines hairline in #1F2937 — only horizontal, "
            "every ~4 label steps. X-axis timestamps in JetBrains Mono "
            "11px text-tertiary. Y-axis prices formatted with thousand "
            "separators. No data points shown, only the line; on hover, "
            "a vertical rule drops and a compact floating tag shows "
            "timestamp + P&L in mono. Plotly dark template as baseline. "
            "RANGE CHIPS: Horizontal chip row immediately below the "
            "chart, right-aligned. Labels: 1D · 1W · 1M · 3M · YTD · ALL. "
            "Each chip is 28px tall, 12px horizontal padding, 8px "
            "radius, text-secondary. Active chip gets a surface fill and "
            "text-primary — no border changes, no accent color. "
            "PANELS ROW: Two equal columns (st.columns([1, 1])) with a "
            "16px gap. Each column is a card: 24px padding, hairline "
            "border, 12px radius. LEFT card titled with 'WATCHING' "
            "(small uppercase eyebrow) and a counter pill like '(3)' to "
            "its right in text-tertiary. Rows inside: left = symbol in "
            "JetBrains Mono 14px text-primary; middle = small label "
            "'last' text-tertiary 11px; right = last price + delta pill "
            "(pill = gain/loss color fill at 16% opacity with full-color "
            "text, e.g. '$43,120 +0.84%'). Entire row is clickable and "
            "navigates to the Asset page for that symbol. Up to 6 rows "
            "visible before the list becomes a scrollable region. Empty "
            "state: centered single line 'No active watchlist' in "
            "text-tertiary, no illustration. RIGHT card titled "
            "'HOLDINGS' with counter. Rows show: symbol (mono) | qty "
            "(mono) | avg cost (mono) | market value (mono) | unrealized "
            "P&L (mono colored). Empty state: 'No open positions'. "
            "Nothing else on the page — no 'Execution mode' caption, no "
            "system-power chrome, no URL blocks. The two-panel row is "
    ...
    },
    {
        ...
        "agent_task": (
            ...
            "Design narrative: The Asset page is chart-first. Operators "
            "come here to see price action and take exactly one decision "
            "(Initialize / Start / Stop); nothing else should compete. "
            "HEADER (single slim row, 56px tall): On the left, a back "
            "chevron Material icon + the word 'Dashboard' as a muted link "
            "in text-secondary — click returns to Home.py. A 16px gap, "
            "then the symbol rendered in JetBrains Mono 24px weight 600 "
            "text-primary (e.g. 'BTC-USD'). Immediately to its right, a "
            "small outlined lifecycle badge — border 1px text-tertiary, "
            "12px padding, 10px radius, 11px uppercase label. For "
            "'uninitialized' the badge reads UNINITIALIZED in text-secondary "
            "on the canvas (outlined). For 'initialized_not_active' it "
            "reads READY with a text-primary fill. For 'active' it reads "
            "ACTIVE with a gain-green 16%-opacity fill and full-color "
            "text. Next to the badge, the watchlist pin: a small star "
            "icon, outlined when unpinned (text-tertiary) and filled in "
            "accent when pinned. Far right of the same row: the single "
            "primary action button that morphs by state. 'Initialize' = "
            "accent indigo fill, 'Start' = gain-green fill, 'Stop' = "
            "loss-red fill. Label changes, color changes, position stays "
            "identical — never more than one primary button visible. "
            "META STRIP (second row, 40px tall, compact): Left-aligned, "
            "a tiny popover trigger styled as a pill that reads 'Mode · "
            "paper' (or 'Mode · live') in text-secondary 12px mono, with "
            "a small caret. Clicking opens an st.popover with the "
            "selectbox + Save button inside — never inline. Just after "
            "the popover, a muted 'Last synced 3s ago' tick in "
            "text-tertiary 11px that updates live from SSE. No other "
            "controls on this strip. "
            "CANDLESTICK HERO: Full width below the meta strip, roughly "
            "480-520px tall. Canvas background (no card), candles use "
            "gain-green for up and loss-red for down, bodies filled, "
            "wicks 1px thin, volume bars at the bottom 20% of the plot "
            "in a heavily muted (~30% opacity) version of the same "
            "colors. Gridlines hairline. No chart title text. Interval "
            "selector sits ABOVE-RIGHT of the chart as a segmented "
            "control with labels 1s · 1m · 1h · 1D · 1W · 1M, same "
            "styling as the range chips on the dashboard. Buy/sell "
            "markers from /assets/chart/trade-markers overlay as tiny "
            "triangles (▲ gain-green for buys, ▼ loss-red for sells) at "
            "the bar close, with hover showing side / qty / correlation "
            "id in a mono tooltip. "
            "EMPTY STATE (unies IL-CHART-001): When the chart has no "
            "data for this symbol, replace the candlestick with a "
            "centered empty-state card inside the chart region — a "
            "small outlined icon (show_chart or timeline), a single "
            "line in text-secondary 'No price history for <SYMBOL> "
            "yet', and the Initialize primary button directly beneath "
            "it. No red banner, no blue info strip. "
            "DEVELOPER / DEBUG: Everything currently rendered as "
            "inline captions — the 'FB-AP-034 — SSE push' URL row, the "
            "'Chart API (read-only)' section with Bars / Trade markers "
            "URLs, and the 'Requires NM_CONTROL_PLANE_API_KEY' hint — "
            "all collapse into a single st.expander labeled 'Developer "
            "/ Debug' with expanded=False, rendered at the BOTTOM of "
            "the page below the chart. Inside the expander, URLs are "
            "in JetBrains Mono 12px inside code blocks, and each is "
            "followed by a small copy-to-clipboard icon button. An "
            "operator in normal use never sees any of it."
        ),
        "affected_files": (
    ...
    },
    {
        ...
        "agent_task": (
            ...
            "Design narrative — LOGIN + SIGN-UP: A logged-out user "
            "never sees app chrome. Both pages call "
            "st.set_page_config(initial_sidebar_state='collapsed') and "
            "hide the sidebar toggle via CSS. The viewport is the full "
            "canvas (#0B0F17). Vertically and horizontally centered on "
            "the viewport sits a single card: max-width 420px, 32px "
            "internal padding, surface #111827 background, 12px corner "
            "radius, 1px border #1F2937, no shadow. Inside the card, "
            "stacked vertically with 16px gaps between blocks: (1) The "
            "brand lockup centered — small outlined mark beside "
            "'trading_bot' in JetBrains Mono 16px weight 500. (2) A "
            "one-line tagline, text-secondary 13px, centered: 'Operate "
            "paper and live trading from one console.' (3) A 32px gap. "
            "(4) The form: email input, password input, each full-width "
            "44px tall, 8px radius, 1px border #1F2937, background "
            "canvas-color so they feel recessed inside the card. Inter "
            "14px text-primary. Labels are 11px uppercase text-tertiary "
            "letter-spacing 0.08em sitting above each input. (5) 16px "
            "gap. (6) Primary CTA button, full-width, 44px tall, accent "
            "#6366F1 fill, text white 14px weight 500, 8px radius — "
            "reads 'Sign in' or 'Create account'. Hover lightens by "
            "8%. (7) 16px gap. (8) One muted link centered in "
            "text-secondary 12px: on the Login page reads 'Don't have "
            "an account? Create one' with 'Create one' underlined on "
            "hover; on the Sign-up page reads 'Already have an account? "
            "Sign in'. No social auth, no 'remember me', no "
            "decorations, no small print except a minimal legal footer "
            "below the card in text-tertiary 11px ('Paper trading only "
            "until live credentials are added.'). Errors render inline "
            "under the relevant field in loss-red 12px; successes in "
            "gain-green. "
            "Design narrative — ACCOUNT PAGE: Sidebar visible (user is "
            "authenticated). Main content is a 720px-wide centered "
            "column. No hero. Three stacked cards (same card styling "
            "as everywhere — surface, hairline border, 12px radius, "
            "24px padding) with 16px vertical gaps between them: "
            "CARD 1 — PROFILE: Eyebrow 'PROFILE' uppercase 11px "
            "text-tertiary. Inside: a two-column label:value grid — "
            "labels on the left in text-tertiary 12px, values on the "
            "right in text-primary 13px (mono for dates). Rows: Email, "
            "Joined, Last login. "
            "CARD 2 — SECURITY: Eyebrow 'SECURITY'. A single primary "
            "outline-style button 'Sign out of all sessions' — text "
            "loss-red, border 1px loss-red at 40% opacity, transparent "
            "fill, hover fills to loss-red at 8% opacity. Under the "
            "button, a small list of recent sessions, each row showing "
            "IP (mono) + device (truncated) + relative time, all in "
            "text-secondary 12px. "
            "CARD 3 — VENUE KEYS: Eyebrow 'VENUE KEYS'. Two sub-rows, "
            "each a horizontal layout: left side shows the venue name "
            "('Alpaca' / 'Coinbase') in Inter 14px text-primary with a "
            "small venue icon; middle shows a masked key suffix '•••• "
            "abcd' in JetBrains Mono 13px text-secondary; right shows "
            "a status pill — 'Active' with gain-green 16% fill + "
            "full-color text if keys are set, 'Not set' with a neutral "
            "#1F2937 fill + text-tertiary if not; far right shows an "
            "'Update' link in accent #6366F1 that deep-links to the "
            "FB-UX-017 wizard starting on that venue's step. A "
            "hairline divider between the two sub-rows. "
            "Nothing else on the Account page. No large unstyled form "
            "fields. No raw JSON dumps. No diagnostic sections."
        ),
        "affected_files": (
    ...
    },
    {
        ...
        "agent_task": "COMPLETED. ServiceConfigurationSnapshot fields + replay_contract threading (pipeline.step, replay_core); DecisionRecord input_snapshot_ids includes service_config_snapshot_id; tests test_service_config_snapshot_fb_can_071.py.",
    ...
    },
    {
        ...
        "agent_task": "COMPLETED. tb_canonical_lag_seconds (domain=data_ingestion|decision|execution_feedback) in observability/lag_metrics.py; run_step observes + Grafana + alerts canonical_system_lag_fb_can_072; tests test_lag_metrics_fb_can_072.py.",
    ...
    },
    {
        ...
        "agent_task": "COMPLETED. session_mode + throttle on CanonicalStateOutput; RiskState occupancy; composite sizing × session_mode_throttle; trigger threshold bumps + trg_session_mode_throttle; Prometheus tb_canonical_session_mode + transition gauge; default.yaml session_mode; tests test_session_mode_fb_can_073.py.",
