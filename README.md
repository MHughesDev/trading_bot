...
Endpoints: `/status`, `/routes`, `/params`, `/system/mode`, `/system/power`, `/system/execution-profile` (GET/POST — paper/live intent + config patch; restart to apply), `/portfolio/positions` (open positions + mark/uPnL: paper defaults to **Kraken mid** via `NM_PORTFOLIO_MARK_PRICE_SOURCE_PAPER`, live defaults to **venue-only** via `NM_PORTFOLIO_MARK_PRICE_SOURCE_LIVE`), `/flatten`, `/models`, `/metrics` (Prometheus), `/microservices/health` (optional scaffold process probes; see `infra/docker-compose.microservices.yml`).
