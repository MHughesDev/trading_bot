...
3. **Optional — `trading_bot_operator_launcher.exe` (PyInstaller)** — after **`setup.bat`**, run **`packaging\windows\build_operator_bundle.bat`** to build a thin console launcher that mirrors **`run.bat`** (spawns the same three processes using **`.venv`**). Place **`dist\trading_bot_operator_launcher\`** next to **`setup.bat`** or run the exe from the repo root. Distribution caveats: **[`docs/WINDOWS_OPERATOR_UI.MD`](docs/WINDOWS_OPERATOR_UI.MD)** (§1.1).
...
**Decision pipeline (only path):** master spec — **`forecaster_model`** → **`ForecastPacket`** → **`PolicySystem`** → risk / execution (see [`docs/Human Provided Specs/MASTER_SYSTEM_PIPELINE_SPEC.MD`](docs/Human%20Provided%20Specs/MASTER_SYSTEM_PIPELINE_SPEC.MD) and [`docs/SYSTEM_WALKTHROUGH.MD`](docs/SYSTEM_WALKTHROUGH.MD)). **Forecaster quantiles:** optional **`NM_MODELS_FORECASTER_TORCH_PATH`** (`forecaster_torch.pt` from **`tb-train-forecaster-distill`** / `train_distilled_mlp_forecaster`) else **`NM_MODELS_FORECASTER_WEIGHTS_PATH`** (NPZ) else NumPy RNG. Policy: **`NM_MODELS_POLICY_MLP_PATH`** (NPZ) else heuristic. Logs `decision pipeline serving mode:` (`pytorch_mlp` / `npz_weights` / `numpy_rng`). Optional: `NM_MODELS_FORECASTER_CHECKPOINT_ID`, `NM_MODELS_FORECASTER_CONFORMAL_STATE_PATH`, **`NM_MODELS_TORCH_DEVICE`**. **Per-asset manifest (optional, FB-AP-001):** set **`NM_ASSET_MODEL_MANIFEST_PATH`** (single JSON) or **`NM_ASSET_MODEL_REGISTRY_PATH`** (directory of per-symbol manifests); when set, the pipeline asserts manifest `symbol` matches the decision tick symbol and abstains with **`NO_TRADE`** on mismatch. Train PyTorch MLP: `pip install -e ".[models_torch]"` then `tb-train-forecaster-distill` (or `python -m forecaster_model.training.distill_mlp`); toy linear smoke: `forecaster_model.training.train_forecaster_torch`. Historical note: [`docs/MIGRATION_TO_SPEC_PIPELINE.MD`](docs/MIGRATION_TO_SPEC_PIPELINE.MD).
...
Endpoints: `/status`, `/routes`, `/params`, `/system/mode`, `/system/power`, `/system/execution-profile` (GET/POST — paper/live intent + config patch; restart to apply), `/portfolio/positions` (open positions + mark/uPnL: paper defaults to **Kraken mid** via `NM_PORTFOLIO_MARK_PRICE_SOURCE_PAPER`, live defaults to **venue-only** via `NM_PORTFOLIO_MARK_PRICE_SOURCE_LIVE`), **`/pnl/summary?range=`** (realized from `data/pnl_ledger.jsonl`, unrealized from positions — [`docs/PNL_LEDGER.MD`](docs/PNL_LEDGER.MD)), `/flatten`, `/models`, `/metrics` (Prometheus), `/microservices/health` (optional scaffold process probes; see `infra/docker-compose.microservices.yml`).
...
**Microservices dev (optional):** `docker compose -f infra/docker-compose.yml -f infra/docker-compose.microservices.yml up -d redis live_runtime market_data_service feature_service decision_service risk_service execution_gateway observability_writer` adds **live_runtime** (8208 — HTTP health only by default; set **`NM_LIVE_SERVICE_APP_START_LOOP=true`** in the service env to run the real Kraken loop), plus **market_data** (8206), **feature** (8205), **decision** (8203), **risk** (8204), **execution gateway** (8202), and **observability writer** (8207 — consumes execution + risk-blocked events). Set **`NM_MARKET_DATA_KRAKEN_WS=true`** on **market_data_service** to publish real Kraken WebSocket ticks to Redis (otherwise use **`POST /ingest/raw-tick`**). **feature_service** defaults to **`RollingBars` + `FeaturePipeline`** (same path as `live_service`; disable with **`NM_FEATURE_SERVICE_USE_PIPELINE=false`**). Optional QuestDB audit: **`NM_QUESTDB_PERSIST_MICROSERVICE_EVENTS=true`** writes subscribed bus events to the **`microservice_events`** table (see `RUNBOOKS`). POST a raw tick to `http://localhost:8206/ingest/raw-tick`, or a normalized tick to `http://localhost:8205/ingest/market-tick`, or a feature row to `http://localhost:8203/ingest/features-row`; poll `http://localhost:8202/events/recent` on the gateway and `http://localhost:8207/events/recent` on the observability writer. **`GET http://localhost:8000/microservices/health`** on the control plane (when running) probes those ports on the host.
