...
**Decision pipeline (only path):** master spec — **`forecaster_model`** (xLSTM stack) → **`ForecastPacket`** → **`PolicySystem`** → risk / execution (see [`docs/Human Provided Specs/MASTER_SYSTEM_PIPELINE_SPEC.MD`](docs/Human%20Provided%20Specs/MASTER_SYSTEM_PIPELINE_SPEC.MD) and [`docs/SYSTEM_WALKTHROUGH.MD`](docs/SYSTEM_WALKTHROUGH.MD)). Default: **NumPy** forecaster forward with **RNG-drawn** weights per layer unless you set **`NM_MODELS_FORECASTER_WEIGHTS_PATH`** (NPZ bundle); policy defaults to **heuristic** unless **`NM_MODELS_POLICY_MLP_PATH`** (NPZ). Startup logs `decision pipeline serving mode:` with `numpy_rng`/`npz_weights` and `heuristic`/`mlp_npz`. Optional: `NM_MODELS_FORECASTER_CHECKPOINT_ID` (lineage label), `NM_MODELS_FORECASTER_CONFORMAL_STATE_PATH` (conformal JSON). PyTorch export to the hot path remains **FB-FR-P0** / training. Historical note: [`docs/MIGRATION_TO_SPEC_PIPELINE.MD`](docs/MIGRATION_TO_SPEC_PIPELINE.MD).
