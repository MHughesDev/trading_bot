...
Endpoints: `/status`, `/routes`, `/params`, `/system/mode`, `/system/power`, `/system/execution-profile` (GET/POST — paper/live intent + config patch; restart to apply), `/flatten`, `/models`, `/metrics` (Prometheus), `/microservices/health` (optional scaffold process probes; see `infra/docker-compose.microservices.yml`).
