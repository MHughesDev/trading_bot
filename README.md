...
**Microservices dev (optional):** `docker compose -f infra/docker-compose.yml -f infra/docker-compose.microservices.yml up -d redis decision_service risk_service execution_gateway` starts Redis plus **decision** (8203), **risk** (8204), and **execution gateway** (8202) with Redis Streams (first start installs deps in each container). POST a feature row to `http://localhost:8203/ingest/features-row` and poll `http://localhost:8202/events/recent` on the gateway.
