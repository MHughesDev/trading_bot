...
3. **Optional — `nm_operator_launcher.exe` (PyInstaller)** — after **`setup.bat`**, run **`packaging\windows\build_operator_bundle.bat`** to build a thin console launcher that mirrors **`run.bat`** (spawns the same three processes using **`.venv`**). Place **`dist\nm_operator_launcher\`** next to **`setup.bat`** or run the exe from the repo root. Distribution caveats: **[`docs/WINDOWS_OPERATOR_UI.MD`](docs/WINDOWS_OPERATOR_UI.MD)** (§1.1).
...
**Decision pipeline (only path):** master spec — **`forecaster_model`** → **`ForecastPacket`** → **`PolicySystem`** → risk / execution (see [`docs/Human Provided Specs/MASTER_SYSTEM_PIPELINE_SPEC.MD`](docs/Human%20Provided%20Specs/MASTER_SYSTEM_PIPELINE_SPEC.MD) and [`docs/SYSTEM_WALKTHROUGH.MD`](docs/SYSTEM_WALKTHROUGH.MD)). **Forecaster quantiles:** optional **`NM_MODELS_FORECASTER_TORCH_PATH`** (`forecaster_torch.pt` from **`nm-train-forecaster-distill`** / `train_distilled_mlp_forecaster`) else **`NM_MODELS_FORECASTER_WEIGHTS_PATH`** (NPZ) else NumPy RNG. Policy: **`NM_MODELS_POLICY_MLP_PATH`** (NPZ) else heuristic. Logs `decision pipeline serving mode:` (`pytorch_mlp` / `npz_weights` / `numpy_rng`). Optional: `NM_MODELS_FORECASTER_CHECKPOINT_ID`, `NM_MODELS_FORECASTER_CONFORMAL_STATE_PATH`, **`NM_MODELS_TORCH_DEVICE`**. Train PyTorch MLP: `pip install -e ".[models_torch]"` then `nm-train-forecaster-distill` (or `python -m forecaster_model.training.distill_mlp`); toy linear smoke: `forecaster_model.training.train_forecaster_torch`. Historical note: [`docs/MIGRATION_TO_SPEC_PIPELINE.MD`](docs/MIGRATION_TO_SPEC_PIPELINE.MD).
