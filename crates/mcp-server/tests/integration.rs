...
use mcp_server_lib::{dispatch_tool, mcp_live_automations_allowed, McpContext};
...
async fn workflow_a_draft_to_finalize() {
    ...
    .await;
    assert_eq!(
        r.get("valid").and_then(|v| v.as_bool()),
        Some(true),
        "finalize failed: {r}"
    );
    let store_id = r
    ...
    .await;
    assert_eq!(
        r.get("discarded").and_then(|v| v.as_bool()),
        Some(false),
        "draft should already be removed"
    );
    ...
    assert!(strategies
        .iter()
        .any(|s| s.get("store_id").and_then(|v| v.as_str()) == Some(&store_id)));
...
async fn workflow_b_invalid_draft_catches_errors() {
    ...
    assert!(
        r.get("error").is_none(),
        "draft should still exist after failed finalize: {r}"
    );
...
fn tool_definitions_is_valid_json_array() {
    ...
    let arr = defs
        .as_array()
        .expect("tool_definitions should be a JSON array");
