...
from .schemas import TrainRequest, TrainResponse, EvalRequest, EvalResponse, EnsembleCombineRequest, EnsembleCombineResponse
...
async def run_ensemble_combine(req: EnsembleCombineRequest) -> EnsembleCombineResponse:
    """Ensemble combine + conformal calibration + scoring pipeline (I-4.11).

    1. Load each member artifact; run batch inference → (N, L) quantile arrays.
    2. Project members to spine σ-coordinates (I-4.8).
    3. Combine via the chosen combiner (I-4.4/I-4.5/I-4.6).
    4. Apply adaptive conformal calibration on the cal rows (I-4.9).
    5. Repair crossings (I-4.10).
    6. Score with the Phase 2 suite (I-4.11).
    7. Persist the combined bundle; return artifact_uri + full metrics/scorecard/report.
    """
    ...
    async def _emit(phase: str, pct: float):
        if subject:
            await publisher.publish(subject, {
                "run_id": req.ensemble_id, "phase": phase, "progress": pct,
            })

    try:
        await _emit("loading_members", 5.0)
        store = get_store()

        raw_dataset = store.get(req.dataset_uri)
        df = pd.read_parquet(io.BytesIO(raw_dataset))

        if "label" not in df.columns:
            return EnsembleCombineResponse(status="failed", error="dataset missing 'label' column")
        realized = df["label"].to_numpy(dtype=np.float64)

        await _emit("running_member_inference", 20.0)

        levels = req.levels or [0.05, 0.1, 0.25, 0.5, 0.75, 0.9, 0.95]
        member_quantiles = []
        member_sigmas = []

        for member in req.roster:
            try:
                artifact_bytes = store.get(member.artifact_uri)
            except Exception as e:
                return EnsembleCombineResponse(status="failed", error=f"member {member.model_ref}: {e}")

            definition = {"output": {"quantile_levels": levels}}
            feature_cols = [c for c in df.columns if c not in ("label", "target")]
            df_feat = df[feature_cols].fillna(0.0)

            q, lvls, _, sigma = _eval_inference(artifact_bytes, df_feat.assign(label=realized), "unknown", True, definition)
            if q is None:
                q = np.zeros((len(realized), len(levels)))
            if len(levels) != q.shape[1]:
                q = np.zeros((len(realized), len(levels)))

            member_quantiles.append(q)
            member_sigmas.append(member.sigma)

        await _emit("combining", 50.0)

        # Build cal_mask from cal_start/cal_end
        cal_mask: np.ndarray | None = None
        if req.cal_end > req.cal_start:
            cal_mask = np.zeros(len(realized), dtype=bool)
            cal_mask[req.cal_start: req.cal_end] = True

        member_crps = [m.crps for m in req.roster if m.crps is not None] or None

        result = ens_mod.combine(
            member_quantiles=member_quantiles,
            member_sigmas=member_sigmas,
            realized=realized,
            levels=levels,
            combiner=req.combiner,
            weight_floor=req.weight_floor,
            temperature=req.temperature,
            cal_mask=cal_mask,
            member_crps=member_crps,
        )

        combined = result["combined"]        # (N, L) return units
        weights = result["weights"]
        crossing_count = result["crossing_count"]
        conf_state = result["conformal_state"]
        stack_state = result["stacking_state"]

        await _emit("scoring", 70.0)
        ...
        metrics["ensemble_id"] = req.ensemble_id
        metrics["combiner"] = req.combiner
        metrics["crossing_count"] = crossing_count
        if weights is not None:
            metrics["weights"] = weights

        scorecard = _build_eval_scorecard(metrics)

        # Persist a minimal ensemble bundle: header + conformal/stacking state.
        import json, struct, hashlib
        import pickle
        bundle_header = {
            "schema_version": "1.0",
            "artifact_kind": "ensemble",
            "ensemble_id": req.ensemble_id,
            "version": req.version,
            "roster": [m.model_ref for m in req.roster],
            "combiner": req.combiner,
            "weight_floor": req.weight_floor,
            "temperature": req.temperature,
            "sigma_spine": member_sigmas[0] if member_sigmas else 1.0,
            "quantile_levels": levels,
            "conformal_state": conf_state,
            "stacking_state": stack_state,
            "weights": weights,
        }
        hb = json.dumps(bundle_header, separators=(",", ":")).encode("utf-8")
        bundle_bytes = struct.pack(">I", len(hb)) + hb

        key = f"ensembles/{req.ensemble_id}/v{req.version}/ensemble.bin"
        uri, sha256, _ = store.put(key, bundle_bytes)
        ...
        await _emit("succeeded", 100.0)
        return EnsembleCombineResponse(
            status="succeeded",
            artifact_uri=uri,
        ...
        await _emit("failed", 100.0)
        return EnsembleCombineResponse(status="failed", error=str(e))
    finally:
        await publisher.close()
