"""
Per-asset initialization pipeline orchestrator (FB-AP-006 / FB-AP-007).

Runs **one symbol at a time** in a background thread. Step **kraken_fetch** uses
:mod:`orchestration.init_kraken_historical` for parameterized Kraken REST bootstrap history.
Step **validate** uses :mod:`data_plane.bootstrap_bars` (FB-AP-008). Later steps are stubbed
until FB-AP-009–012.
"""
...
def _pipeline_body(job_id: str, symbol: str) -> None:
    ...
    def do_validate() -> tuple[str, str | None]:
        ...
        v = validate_and_clean_init_bootstrap_bars(
