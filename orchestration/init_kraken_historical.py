...
Fetches OHLCV for the operator symbol (e.g. ``BTC-USD``) using the shared
:func:`orchestration.real_data_bars.fetch_symbol_bars_sync` path — same Kraken-only policy
as the rest of the repo. Pair mapping (display symbol → Kraken REST ``pair`` / wsname) is
documented in returned metadata; HTTP retries/rate-limit handling live in
:class:`data_plane.ingest.kraken_rest.KrakenRESTClient`.
"""
