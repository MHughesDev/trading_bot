...
export type AIForecastNodeData = { model: string; direction: ForecastDirection; minConfidence: number; alias?: string; disabled?: boolean }
  ...
  return (
    <div className={`tb-node${selected ? ' selected' : ''}${data.disabled ? ' disabled' : ''}`} style={{ minWidth: 230 }}>
      ...
      <div className="tb-node-body">
        ...
        <div className="tb-node-row">
          <span className="tb-node-label">Alias</span>
          <select className="tb-select" value={data.alias ?? 'production'} onChange={e => updateNodeData(id, { alias: e.target.value })}>
