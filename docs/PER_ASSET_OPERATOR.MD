...
- **Account (FB-UX-005)** — Sidebar **Account** link (**`:material/settings:`**) → **`pages/7_Account.py`** (placeholder; venue key forms are **FB-UX-006**). Shared chrome: **`control_plane/streamlit_chrome.py`** **`render_app_sidebar()`**.
