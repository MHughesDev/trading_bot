...
| [`app/contracts/hard_override.py`](../app/contracts/hard_override.py), [`decision_engine/state_engine.py`](../decision_engine/state_engine.py) | **Hard override taxonomy + degradation accounting** — **FB-CAN-033**; `classify_hard_override`, `RiskState` transition/occupancy, `forecast_diagnostics["hard_override"]`. |
...
| [`decision_engine/auction_engine.py`](../decision_engine/auction_engine.py) | **APEX opportunity auction** — **FB-CAN-006**; **FB-CAN-034** correlation/thesis/liquidation diversification penalties + **`clustering_metadata`**; `AuctionResult` in `ForecastPacket.forecast_diagnostics["auction"]`. |
