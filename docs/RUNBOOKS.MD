...
- **`NM_EXECUTION_MODE`** is read at **process start** from environment (and `default.yaml` via `load_settings`). It is **not** hot-swapped while `live_service` runs.
- **Operator flow:** `GET /status` includes **`execution_profile`** (`active_execution_mode`, `pending_execution_mode`, `restart_required`). `POST /system/execution-profile` with `{"execution_mode":"paper"|"live"}` records intent and, by default, patches **`app/config/default.yaml`** and **`.env`** (`NM_EXECUTION_MODE=...`) when those files exist.
...
5. **Restart** all runtime processes after Apply so **`NM_EXECUTION_MODE=live`** loads; verify **`GET /status` → `execution_profile`** shows no unexpected **pending** mismatch.
...
2. If you use **`python -m app.runtime.power_supervisor`**, restart it so it spawns a fresh **`live_service_app`** with the new environment (or toggle system power OFF then ON after updating `.env`).
...
- **Runtime vs training:** campaign **QuantileRegressor** joblib is still **not** the same object as the NumPy `ForecasterModel` forward; NPZ weights align the **reference** forecaster with a saved draw. **PyTorch** checkpoints remain training-side until a torch export path is added (**FB-FR-P0**; delivered NumPy path: **FB-FR-CORE**). On startup, look for log line `decision pipeline serving mode:` (`numpy_rng` vs `npz_weights`, `heuristic` vs `mlp_npz`; see [`README.md`](../README.md)).
...
- **Operator-facing labels:** set **`NM_MODELS_FORECASTER_CHECKPOINT_ID`** for `ForecastPacket` lineage; use **`POST /models/version`** on the control plane to expose **`component` + `version`** to Prometheus (`nm_model_version_info`). Together with **`model_artifacts`** and artifact paths, this is the current “active model set” surface — a single MLflow-backed promotion workflow remains backlog.
...
6. **Monitor** first session: Grafana/Loki, `nm_decision_latency_seconds`, adapter errors.
