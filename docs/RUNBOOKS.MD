...
- **`GET /status` → `model_artifacts`**: single JSON object with **`active_model_set`** (manifest path / existence / label / version), **serving** paths (`NM_MODELS_*` for NPZ, conformal JSON, lineage id) with **`file_exists`** flags, plus a **training** note that `forecaster_quantile_real.joblib` is **campaign-only** (sklearn QuantileRegressor), not the NumPy hot path. Implemented in `app/config/model_artifacts.py` and `models/registry/active_set.py`.
