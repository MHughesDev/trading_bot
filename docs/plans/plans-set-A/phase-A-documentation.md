...
- Read the template thoroughly first: the template's `docs/README.md` and
  ...
  artifact's `SC-N`/`FM-N` IDs, the open-questions `OQ-NNN` register, the plan **Derived From**
  traceability).
...
- **Follow the template's conventions exactly.** Spec IDs are `<TYPE>-<NNN>`; ADRs are
  ...
  `OQ-NNN`; never renumber, always append.
...
- **Files:** copy the template's `docs/` → repo-root `docs/`. Keep verbatim:
  ...
  empty-but-templated starting files: `docs/artifact.md`, `docs/research/OPEN_QUESTIONS.md`,
  ...
  rows in `research/OPEN_QUESTIONS.md` and each index README.
...
### P A-T03 — Migrate the open-questions register (`docs/research/OPEN_QUESTIONS.md`)
- **Goal:** Port the spec's open questions into the `OQ-NNN` register with status, options, resolution,
  and evidence.
- **Files:** `docs/research/OPEN_QUESTIONS.md`.
- **Context:** From `[deleted - see Phase 7]/spec/10-open-questions.md`: Q1 (real-vs-paper),
  ...
  fidelity), Q7 (watermark defaults), Q8 (retention) become `OQ-001…OQ-008`. Mark **OQ-001, OQ-002,
  and OQ-003** as **Closed** with the deciding ADR linked (created in P A-T04); the rest **Open** with
  their options recorded. OQ-001 resolution: internal paper fill simulator per asset class (ADR-0006,
  supersedes Alpaca paper account). OQ-002 resolution: Coinbase=live, Kraken=crypto-data,
  Alpaca data feed=equity market data; pipelines start on demand only (ADR-0006, ADR-0011).
  OQ-003 resolution: format frozen in Phase 0 (ADR-0007).
- **Acceptance:** every spec/plan reference to a decision maps to an `OQ-NNN` row; closed rows link
  their ADR and conclusion C-NNN; no example rows remain.
- **Depends on:** P A-T01.
...
- **Context:** Mine the specs for the decisions and write one ADR each (use the ADR template in
  ...
  - `0006-three-system-broker-architecture-coinbase-alpaca-market-simulator` (resolves OQ-001 AND OQ-002:
    Coinbase=live execution all assets, internal paper fill simulator=paper execution, market_simulator
    removed (backtest out of scope); Kraken=crypto market data, Alpaca data feed=equity market data;
    from `[deleted - see Phase 7]/spec/05-execution-and-risk.md`/`10-open-questions.md`)
  ...
  - `0007-freeze-strategy-definition-format-v1` (resolves OQ-003, from `[deleted - see Phase 7]/spec/04-strategy-system.md`)
...
- **Acceptance:** each ADR is `Accepted`, follows the template sections, and lists alternatives; the
  `adr/README.md` index has a row per ADR; OQ-001/OQ-003 in open-questions link to ADR-0006/ADR-0007.
- **Depends on:** P A-T03.
...
- **Context:** Recommended mapping (split further if a file would exceed ~7 cohesive sections):
  ...
  - `[deleted - see Phase 7]/spec/09-tech-stack.md` becomes **conclusions in `CONCLUSIONS.md`** (P A-T06) + the ADRs,
  ...
  initialize. Filling those in is Phase 7's job. §7 references the relevant `OQ-NNN`.
...
### P A-T06 — Write research conclusions to the ledger (`docs/research/CONCLUSIONS.md`)
- **Goal:** Capture the evaluations that justified the stack and the gating venue choice as ledger rows.
- **Files:** `docs/research/CONCLUSIONS.md` (new C-NNN rows), `docs/research/OPEN_QUESTIONS.md`
  (close OQ-001 and OQ-002).
- **Context:** Add conclusions sourced from `[deleted - see Phase 7]/spec/09-tech-stack.md`
  covering stack rationale — cited by ADR-0003/0004 and the tech ADRs. Add conclusions for the
  resolved broker/venue selections (OQ-001 and OQ-002): Coinbase=live execution, internal paper
  fill simulator=paper execution, Kraken WS=crypto market data, Alpaca data feed=equity market
  data. Record rationale, alternatives considered, and the demand-driven pipeline constraint.
  Cite ADR-0006 + ADR-0011. Use `procedures/research-ledger.md`.
- **Acceptance:** conclusions are in CONCLUSIONS.md with source-backed rationale; stack conclusions
  are cited by the relevant ADRs; OQ-001 and OQ-002 are closed referencing the new C-NNN IDs.
- **Depends on:** P A-T04.
...
- **Context:** Fill the template's sections (Overview, Components, Data Flow, External Dependencies,
  ...
  Dependencies link the relevant conclusions in `CONCLUSIONS.md` + ADRs.
...
- **Context:** Check: every `OQ-NNN`/`SC-N`/`FM-N`/`ADR-NNNN`/`§N.M` reference resolves; every plan
...
- [ ] `artifact.md` (with `SC-N`/`FM-N`), `research/OPEN_QUESTIONS.md` (OQ-001 through OQ-068), and `architecture.md`
...
- [ ] Every decided choice is an `Accepted` ADR (11 ADRs total, including ADR-0006 three-system
      ...
      to a template spec with `<TYPE>-<NNN>` ID and `Status: Draft`; relevant tech + venue conclusions exist in `CONCLUSIONS.md`.
