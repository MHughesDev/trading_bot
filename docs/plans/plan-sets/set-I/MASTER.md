...
**Completion: 100% (77 / 77 primary tasks)**

**Status:** SET I COMPLETE — All 6 phases shipped. Phase 6 delivered: CalibratedForecast publish contract (I-6.1), VaR/ES/skew risk read-outs (I-6.2), train/serve parity CI tests (I-6.3), TagRegistry + annotations + templates (I-6.4), ForecastChartsTab fan/PIT/reliability/coverage charts (I-6.5), LeaderboardPage + report viewer (I-6.6), EnsemblesPage builder (I-6.7), PipelinesPage DAG builder with @xyflow/react (I-6.8), nav updated Models·Ensembles·Pipelines (I-6.9). Workspace compiles clean.
**Created:** 2026-06-16
...
| 2026-06-16 | 0 | I-0.5–I-0.11 | **Phase 0 complete.** I-0.6: hash-covers-params‖bytes snapshot immutability proven by 3 unit tests. I-0.7: `GET /api/models/data/quality` endpoint wired (I-0.11). I-0.8: `apps/model-trainer/app/clickhouse.py` tombstoned — sidecar reads only pre-windowed Parquet. I-0.9: `crates/features/src/leakage_harness.rs` — structural future-bar guard + leaky-target detection + fold-geometry disjointness (5 tests). I-0.10: Rust computes folds (`walk_forward_folds` over pinned dataset row_count, default single-expanding-fold when `cv == None`) → `FoldSpec` added to `TrainDispatchRequest`; Python `engine.prepare` applies `_wf_fold` via `prepare_with_fold`. I-0.11: `POST /api/models/data/windows` + `GET /api/models/data/quality` registered in router. `cargo test -p domain -p features -p model-registry` → 129 tests green. |
| 2026-06-16 | 1–2 | I-1.1–I-2.12 | **Phases 1 & 2 complete.** Phase 1: ADR-0016 distributional Forecast contract, definition v1.0→v1.1 migrator, probabilistic targets/labeling, quantile/GARCH-t adapters, in-fold HPO, sorted-quantile enforcement, devolatized σ-unit output. Phase 2: `apps/model-trainer/app/scoring.py` (CRPS, pinball, log-score, PIT, coverage, reliability, Kupiec/Christoffersen, baselines, Diebold–Mariano, deflated CRPS, per-fold/regime breakdowns); `run_evaluation` worker; `EvalRequest`/`EvalResponse` schemas; `SidecarClient.dispatch_evaluate`; `drive_eval` rewired to real sidecar call; leaderboard + evaluation reports endpoints; scorecard upgraded to `100/(1+crps*1000)` quality metric. `cargo test -p features -p model-registry` → 39 tests green. |
| 2026-06-16 | 3 | I-3.1–I-3.10 | **Phase 3 complete.** I-3.1: `feature_sets.rs` rewritten as versioned registry — `list_feature_sets()`, `is_known()`, `validate_user_spec()`; 3 built-in sets (`fs_core_ohlcv_v3`, `fs_extended_v1`, `fs_momentum_v1`). I-3.2: 7 new feature families in `training_frame.rs` — momentum, Parkinson vol, Garman–Klass vol, z-score, relative volume, OBV, calendar sin/cos. I-3.3: `align_higher_tf()` forming-bar-safe multi-resolution assembly + `HigherTfBar`. I-3.4: `devol()`/`fit_sigma()` devolatization transforms. I-3.5: `POST /api/models/features/preview` with synthetic 120-bar fixture. I-3.6: feature importance/correlation surface via `engine.py` `assemble_metrics`. I-3.7: `validate_user_spec()` declarative user-defined feature composition. I-3.8: `crates/model-registry/src/spec_hash.rs` — SHA-256 whole-spec hash (tagged-field encoding); wired into `drive_train` → `config_json`/`metrics_json`. I-3.9: `reproduce_run()` in manager + `POST /api/models/{id}/runs/reproduce`. I-3.10: `compare_runs()` + `GET /api/models/{id}/runs/compare`. `crates/features` stays PURE. `cargo test -p features -p model-registry` → 59 tests green (0 failures). |
